...
public interface BlogCommentMapper {
    ...
    @Select("SELECT * FROM api_blog_comment WHERE blog_id = #{blog_id}")
    List<BlogCommentEntity> selectCommentByBlogId(@Param("blog_id") String blog_id);
    @Select("SELECT * FROM api_blog_comment WHERE author = #{author}")
    List<BlogCommentEntity> selectCommentByAuthor(@Param("author") String author);

    @Insert("INSERT INTO api_blog_comment (comment_id, blog_id, author, content) VALUES (#{comment_id}, #{blog_id}, #{author}, #{content})")
